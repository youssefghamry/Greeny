		...
		data-details="<?php
			echo esc_attr( '<div class="post_details">'
								...
								. '<div class="post_description">'
									. $details
									. ( empty( $greeny_template_args['hide_excerpt'] )
										? '<div class="post_description_content">' . get_the_excerpt() . '</div>'
										: ''
										)
									. '<a href="' . esc_url( $link ) . '" class="theme_button post_readmore"><span class="post_readmore_label">' . esc_html__( 'Learn more', 'greeny' ) . '</span></a>'
								. '</div>'
							. '</div>'
						);
		?>"
	<?php }
    $featured = get_the_post_thumbnail_url(null, greeny_get_thumb_size('square'));
    ?>
>
    <div class="post_content_wrap" <?php if(!empty($featured)) { ?> data-mouse-helper="hover" data-mouse-helper-mode="normal" data-mouse-helper-image="<?php echo esc_url($featured); ?>" <?php } ?>>
        <?php
