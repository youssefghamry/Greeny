...
if (icons.length > 0) {
	...
	#><div id="{{ id }}" class="<# print( trx_addons_apply_filters('trx_addons_filter_sc_classes', 'sc_socials' + (settings.type == 'extra' ? ' extra' : '') + (settings.type == 'alter' ? ' alter' : '') + (settings.type == 'modern' ? ' modern' : '') + (settings.type == 'modern_2' ? ' modern_2' : '') + (settings.type == 'simple' ? ' simple' : '') + ' sc_socials_' + (settings.type == 'alter' || settings.type == 'extra' ? settings.type = 'icons_names' : (settings.type == 'modern' || settings.type == 'modern_2' || settings.type == 'simple') ? settings.type = 'default' : settings.type) + (settings.align != '' ? ' sc_align_'+settings.align : ''), settings ) ); #>">
	
		<?php $element->sc_show_titles('sc_socials'); ?>
	
		<div class="socials_wrap {{ settings.icons_type }}_wrap"><#

        if( 'alter' == settings.type ) {
            settings.type = 'icons_names';
        }
        if ('modern' == settings.type || 'modern_2' == settings.type ) {
