<div class="front_page_section front_page_section_features<?php
	$greeny_scheme = greeny_get_theme_option( 'front_page_features_scheme' );
	if ( ! empty( $greeny_scheme ) && ! greeny_is_inherit( $greeny_scheme ) ) {
		echo ' scheme_' . esc_attr( $greeny_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( greeny_get_theme_option( 'front_page_features_paddings' ) );
	if ( greeny_get_theme_option( 'front_page_features_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$greeny_css      = '';
		$greeny_bg_image = greeny_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$greeny_anchor_icon = greeny_get_theme_option( 'front_page_features_anchor_icon' );
	$greeny_anchor_text = greeny_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( greeny_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$greeny_css      = '';
			$greeny_bg_mask  = greeny_get_theme_option( 'front_page_features_bg_mask' );
			$greeny_bg_color_type = greeny_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $greeny_bg_color_type ) {
				$greeny_bg_color = greeny_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$greeny_caption = greeny_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $greeny_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $greeny_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $greeny_caption, 'greeny_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$greeny_description = greeny_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $greeny_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $greeny_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $greeny_description ), 'greeny_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_features_output">
				...
				if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
					...
					if ( ! greeny_exists_trx_addons() ) {
						...
						greeny_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
					}
				}
				?>
			</div>
