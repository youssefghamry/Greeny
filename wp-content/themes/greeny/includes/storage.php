...
if ( ! function_exists( 'greeny_storage_get' ) ) {
	/**
	 ...
	 */
	function greeny_storage_get( $var_name, $default = '' ) {
		global $GREENY_STORAGE;
		return isset( $GREENY_STORAGE[ $var_name ] ) ? $GREENY_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'greeny_storage_set' ) ) {
	/**
	 ...
	 */
	function greeny_storage_set( $var_name, $value ) {
		global $GREENY_STORAGE;
		$GREENY_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'greeny_storage_empty' ) ) {
	/**
	 ...
	 */
	function greeny_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $GREENY_STORAGE;
		if ( ! empty( $key ) && ! empty( $key2 ) ) {
			return empty( $GREENY_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( ! empty( $key ) ) {
			return empty( $GREENY_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $GREENY_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'greeny_storage_isset' ) ) {
	/**
	 ...
	 */
	function greeny_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $GREENY_STORAGE;
		if ( ! empty( $key ) && ! empty( $key2 ) ) {
			return isset( $GREENY_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( ! empty( $key ) ) {
			return isset( $GREENY_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $GREENY_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'greeny_storage_unset' ) ) {
	/**
	 ...
	 */
	function greeny_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $GREENY_STORAGE;
		if ( ! empty( $key ) && ! empty( $key2 ) ) {
			unset( $GREENY_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( ! empty( $key ) ) {
			unset( $GREENY_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $GREENY_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'greeny_storage_inc' ) ) {
	/**
	 ...
	 */
	function greeny_storage_inc( $var_name, $value = 1 ) {
		global $GREENY_STORAGE;
		if ( empty( $GREENY_STORAGE[ $var_name ] ) ) {
			$GREENY_STORAGE[ $var_name ] = 0;
		}
		$GREENY_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'greeny_storage_concat' ) ) {
	/**
	 ...
	 */
	function greeny_storage_concat( $var_name, $value ) {
		global $GREENY_STORAGE;
		if ( empty( $GREENY_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'greeny_storage_get_array' ) ) {
	/**
	 ...
	 */
	function greeny_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $GREENY_STORAGE;
		if ( empty( $key2 ) ) {
			return ! empty( $var_name ) && ! empty( $key ) && isset( $GREENY_STORAGE[ $var_name ][ $key ] ) ? $GREENY_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && ! empty( $key ) && isset( $GREENY_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $GREENY_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'greeny_storage_set_array' ) ) {
	/**
	 ...
	 */
	function greeny_storage_set_array( $var_name, $key, $value ) {
		global $GREENY_STORAGE;
		if ( ! isset( $GREENY_STORAGE[ $var_name ] ) ) {
			$GREENY_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$GREENY_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'greeny_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function greeny_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $GREENY_STORAGE;
		if ( ! isset( $GREENY_STORAGE[ $var_name ] ) ) {
			$GREENY_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $GREENY_STORAGE[ $var_name ][ $key ] ) ) {
			$GREENY_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$GREENY_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$GREENY_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'greeny_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function greeny_storage_merge_array( $var_name, $key, $value ) {
		global $GREENY_STORAGE;
		if ( ! isset( $GREENY_STORAGE[ $var_name ] ) ) {
			$GREENY_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$GREENY_STORAGE[ $var_name ] = array_merge( $GREENY_STORAGE[ $var_name ], $value );
		} else {
			$GREENY_STORAGE[ $var_name ][ $key ] = array_merge( $GREENY_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'greeny_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function greeny_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $GREENY_STORAGE;
		if ( ! isset( $GREENY_STORAGE[ $var_name ] ) ) {
			$GREENY_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			greeny_array_insert_after( $GREENY_STORAGE[ $var_name ], $after, $key );
		} else {
			greeny_array_insert_after( $GREENY_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'greeny_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function greeny_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $GREENY_STORAGE;
		if ( ! isset( $GREENY_STORAGE[ $var_name ] ) ) {
			$GREENY_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			greeny_array_insert_before( $GREENY_STORAGE[ $var_name ], $before, $key );
		} else {
			greeny_array_insert_before( $GREENY_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'greeny_storage_push_array' ) ) {
	/**
	 ...
	 */
	function greeny_storage_push_array( $var_name, $key, $value ) {
		global $GREENY_STORAGE;
		if ( ! isset( $GREENY_STORAGE[ $var_name ] ) ) {
			$GREENY_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $GREENY_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $GREENY_STORAGE[ $var_name ][ $key ] ) ) {
				$GREENY_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $GREENY_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'greeny_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function greeny_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $GREENY_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $GREENY_STORAGE[ $var_name ] ) && is_array( $GREENY_STORAGE[ $var_name ] ) && count( $GREENY_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $GREENY_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $GREENY_STORAGE[ $var_name ][ $key ] ) && is_array( $GREENY_STORAGE[ $var_name ][ $key ] ) && count( $GREENY_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $GREENY_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'greeny_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function greeny_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $GREENY_STORAGE;
		if ( ! isset( $GREENY_STORAGE[ $var_name ] ) ) {
			$GREENY_STORAGE[ $var_name ] = array();
		}
		if ( empty( $GREENY_STORAGE[ $var_name ][ $key ] ) ) {
			$GREENY_STORAGE[ $var_name ][ $key ] = 0;
		}
		$GREENY_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'greeny_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function greeny_storage_concat_array( $var_name, $key, $value ) {
		global $GREENY_STORAGE;
		if ( ! isset( $GREENY_STORAGE[ $var_name ] ) ) {
			$GREENY_STORAGE[ $var_name ] = array();
		}
		if ( empty( $GREENY_STORAGE[ $var_name ][ $key ] ) ) {
			$GREENY_STORAGE[ $var_name ][ $key ] = '';
		}
		$GREENY_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'greeny_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function greeny_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $GREENY_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $GREENY_STORAGE[ $var_name ] ) ? $GREENY_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $GREENY_STORAGE[ $var_name ] ) ? $GREENY_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'greeny_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function greeny_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $GREENY_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $GREENY_STORAGE[ $var_name ]->$prop ) ? $GREENY_STORAGE[ $var_name ]->$prop : $default;
	}
}
