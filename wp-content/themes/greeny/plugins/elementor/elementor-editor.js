...
jQuery( document ).ready(
	function() {
		...
		greeny_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="greeny_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.greeny_post_editor' );
						if ( link.length > 0 ) {
							greeny_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.greeny_post_editor', function(e) {
						greeny_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('greeny_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function greeny_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
