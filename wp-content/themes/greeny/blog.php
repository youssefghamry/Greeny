...
if ( function_exists( 'greeny_elementor_is_preview' ) && greeny_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			greeny_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	greeny_new_main_query(
		array(
			'post_type'      => greeny_get_theme_option( 'post_type' ),
			'category'       => greeny_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'greeny_filter_get_template_part', greeny_blog_archive_get_template() ) );
}
